...
*   **Limites de Execução Planejada:** Jules Dev não deve executar fases futuras mesmo que os arquivos de planejamento (ex: Fase 18, 19) já existam. O planejamento de fases futuras representa apenas uma fila e direção visualizada, não é uma autorização irrestrita de implementação. Cada fase técnica deve ser executada individualmente, seguida de merge e revisão, antes de iniciar o prompt da próxima.
