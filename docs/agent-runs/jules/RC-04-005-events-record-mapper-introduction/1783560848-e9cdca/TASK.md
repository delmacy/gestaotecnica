# RC-04-005-events-record-mapper-introduction - RC-04-005 Events record mapper introduction

        Objective: introduce a pure exported `mapEventRow` mapper in `events.repository.ts` that maps a camelCase row to `EventRecord` without changing repository call behavior yet.
