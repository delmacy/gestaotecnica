# UX-NAV-03-042-queue-search-draft-recovery-core-domain - Queue, search, and draft recovery complete the operator loop - Core domain model

        Implement the core domain model stage for `Queue, search, and draft recovery complete the operator loop` in UX-NAV-03.
...
Stage objective: Implement or harden domain entities, invariants, state machine rules, workspace scoping, and lifecycle semantics for the slice.
