# UX-NAV-03-044-queue-search-draft-recovery-usecase-api - Queue, search, and draft recovery complete the operator loop - Use case and API binding

        Implement the use case and api binding stage for `Queue, search, and draft recovery complete the operator loop` in UX-NAV-03.
...
Stage objective: Expose the product behavior through server actions, route handlers, services, or API use cases with auth/workspace checks.
