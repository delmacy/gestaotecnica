...
- Start from current origin/main and record the base SHA; do not assume unmerged local Builder portfolio changes exist.
- No mock, demo, synthetic, localStorage-only, or in-memory success may satisfy persistence acceptance.
- Every create/update/publish/execute action must be reloaded from the database in a new request before it counts as evidence.
- All records and actions must be scoped to authenticated organization/workspace membership; never trust a client-supplied userId.
- The PR must name the visible route, action/button, database rows, API/use case, and reproducible verification command.
- No fake assertions, assert.ok(true, hidden fallback, or documentation-only completion for an implementation stage.
- Keep the task serial and within allowedFiles; if an adjacent layer is missing, record the exact blocker for the owning next stage.
