...
## Rastreabilidade
As leituras usam schemas separados de Platform e Runtime dentro do banco
unificado `tec_db`. Instalações de capabilities persistem em
`workspace_module_configs`; processos e flows vêm do schema `workflow`;
entidades vêm do schema `workspace`; capabilities disponíveis vêm do schema
`registry`. A timeline usa eventos e execuções reais do workspace selecionado.
