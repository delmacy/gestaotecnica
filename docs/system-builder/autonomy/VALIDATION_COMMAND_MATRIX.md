...
| `npm run test:unit` | Executar testes unitários isolados do código. | Node.js instalado, sem necessidade de DB. | Relatório no terminal indicando testes passando. | Alterações de regras de negócio puras ou funções isoladas. |
...
| `npm run readiness verify` *(Planejado)* | Check geral de integridade da branch. | N/A (Este comando valida o todo). | Resumo de readiness com sucesso total. | PRs documentais e PRs de código na etapa final pré-merge. |

## Instruções para Testers e Reviewers

Sempre que a infraestrutura permitir, **Tester e Reviewer devem ler relatórios do GitHub Actions e Checks Automáticos** (quando disponíveis). Actions servem como evidência automatizada sujeita ao workflow, configuração e permissões observadas.
