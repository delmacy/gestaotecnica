...
export async function generateTaskKit(workerKey: string, packageKey: string): Promise<TaskKit | null> {
  const worker = await agentWorkDb.select().from(agentWorkers).where(eq(agentWorkers.key, workerKey));
  if (worker.length === 0) return null;

  const pkgRes = await agentWorkDb.select().from(agentWorkPackages).where(eq(agentWorkPackages.key, packageKey));
  ...
  // Fetch tasks
  const tasks = await agentWorkDb.select().from(agentPackageTasks).where(eq(agentPackageTasks.packageKey, packageKey)).orderBy(asc(agentPackageTasks.order));
  ...
  return {
    ...
    dependencies: [], // Fetched in real impl
    ...
    receiptPath: `docs/modules/${pkg.moduleKey}/activity/2024/${pkg.key}__PR-PENDING.md`,
    completionCommands: [
      `npm run agent-work -- receipt:activity ${pkg.key}`,
