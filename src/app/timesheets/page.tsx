...
import { TechnicianTimeList } from "@/modules/timesheets/technician-time-list";
import { TimesheetTable } from "@/modules/timesheets/timesheet-table";
import {
  getTechnicianTimeSummary,
  getTimesheetEntries,
  getTimesheetSummary,
} from "@/modules/timesheets/queries";

export const dynamic = "force-dynamic";

export default async function TimesheetsPage() {
  const [entries, summary, technicians] = await Promise.all([
    getTimesheetEntries(),
    getTimesheetSummary(),
    getTechnicianTimeSummary(),
  ]);
    ...
    <main className="min-h-screen bg-[#f6f7f4] text-[#1c211b]">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm leading-6 text-[#4d5848]">
                Consolidacao dos apontamentos de tempo lancados nas ordens de
                servico.
              </p>
            </div>
            <Link
      ...
      <section className="mx-auto grid w-full max-w-7xl gap-6 px-6 py-8 lg:grid-cols-[1fr_420px] lg:px-8">
        ...
        <aside>
          ...
          <TechnicianTimeList technicians={technicians} />
        </aside>
      </section>
    </main>
  );
