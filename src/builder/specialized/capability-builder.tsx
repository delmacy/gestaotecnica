...
import { Layers, Zap, History, Database, Layout } from "lucide-react";
import { cn } from "@/lib/utils";

export function CapabilityBuilder({ activeItem }: { activeItem: any }) {
  const meta = activeItem.metadata || {};
  const actions: Array<{ key: string; description?: string }> = Array.isArray(meta.actions) ? meta.actions : [];
  const entities: string[] = Array.isArray(meta.entities) ? meta.entities : [];
  const events: string[] = Array.isArray(meta.events) ? meta.events : [];
  const views: string[] = Array.isArray(meta.views) ? meta.views : [];
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        <div className="flex items-start justify-between border-b pb-6">
          ...
          <div className="flex flex-col items-end gap-2">
             <div className="text-[10px] font-mono text-muted-foreground bg-muted px-2 py-0.5 rounded">{meta.source || "client_state"}</div>
          </div>
        </div>

        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {[
            { label: "Atomic Actions", value: String(actions.length), icon: Zap, color: "text-amber-500" },
            { label: "Event Definitions", value: String(events.length), icon: History, color: "text-blue-500" },
            { label: "Data Entities", value: String(entities.length), icon: Database, color: "text-emerald-500" },
            { label: "UI Templates", value: String(views.length), icon: Layout, color: "text-purple-500" },
          ].map(stat => (
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
             {(entities.length ? entities : ["Nenhuma entidade vinculada"]).map((entity: string) => (
               <div key={entity} className="flex items-center justify-between p-4 border rounded-xl bg-white hover:border-primary transition-colors group">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-muted-foreground italic">{meta.source || "Sem origem persistida"}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <Badge variant="outline" className="text-[9px]">{meta.status || "registry"}</Badge>
                  </div>
               </div>
             ))}
          </div>
        ...
        <div className="grid gap-8 md:grid-cols-2 mt-12">
          <section className="space-y-4">
            ...
            <div className="bg-muted/20 rounded-2xl p-4 border border-dashed space-y-2">
               {(actions.length ? actions : [{ key: "Nenhuma action registrada" }]).map((action: any) => (
                 <div key={action.key} className="text-xs font-mono p-2 hover:bg-white rounded transition-colors">
                    <span className="font-bold">{action.key}</span>
                    {action.description && <p className="text-[10px] text-muted-foreground mt-1 font-sans">{action.description}</p>}
                 </div>
               ))}
            </div>
          </section>
          ...
          <section className="space-y-4">
            ...
            <div className="bg-muted/20 rounded-2xl p-4 border border-dashed space-y-2">
               {(events.length ? events : ["Nenhum evento vinculado"]).map(event => (
