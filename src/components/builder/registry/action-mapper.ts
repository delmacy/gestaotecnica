...
export function mapActionToRegistryItem(action: ActionDefinition<any, any>): RegistryItem {
  ...
  return {
    ...
    name: action.uiLabel || action.key.split('.').map((part: string) => part.charAt(0).toUpperCase() + part.slice(1)).join(' '),
    slug: action.key,
    type: 'action',
