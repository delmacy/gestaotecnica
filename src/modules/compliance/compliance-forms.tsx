  ...
  return (
    <form action={createComplianceAudit} className="border border-[#d7dccf] bg-white p-5 shadow-sm">
      ...
      <div className="mt-4 space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <select className="h-11 border border-[#c8d0bf] bg-[#fbfcf8] px-3 text-sm" name="ownerTeamId" defaultValue=""><option value="">Sem equipe dona</option>{options.teams.map((t: any) => <option key={t.id} value={t.id}>{t.name}</option>)}</select>
          <select className="h-11 border border-[#c8d0bf] bg-[#fbfcf8] px-3 text-sm" name="assetId" defaultValue=""><option value="">Sem ativo</option>{options.assets.map((a: any) => <option key={a.id} value={a.id}>{a.code} - {a.name}</option>)}</select>
        </div>
        <div className="grid gap-3 sm:grid-cols-2">
  ...
  return (
    <form action={createComplianceFinding} className="border border-[#d7dccf] bg-white p-5 shadow-sm">
      ...
      <div className="mt-4 space-y-4">
        ...
        <select className="h-11 w-full border border-[#c8d0bf] bg-[#fbfcf8] px-3 text-sm" name="responsibleTeamId" defaultValue=""><option value="">Sem equipe responsavel</option>{options.teams.map((t: any) => <option key={t.id} value={t.id}>{t.name}</option>)}</select>
