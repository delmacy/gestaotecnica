...
export async function cleanLaunchDemo(
  ...
  const orgKeys = [LAUNCH_DEMO.organization.key];
  const moduleKeys = [LAUNCH_DEMO.module.key];
  ...
  console.log(`Cleanup finished for Launch Demo`);
}
...
async function runCleanScript() {
  ...
  try {
    ...
    await cleanLaunchDemo(dbPlatform, dbRuntime);
  } catch (error) {
