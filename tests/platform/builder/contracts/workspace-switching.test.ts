...
describe('Workspace Switching Contract', () => {
    ...
    it('should forbid switching to unauthorized workspace', async () => {
        ...
        const response = await resolveWorkspaceSwitching({ currentWorkspaceId: 'ws-1', targetWorkspaceId: 'forbidden-ws', organizationId: 'org-1', userId: 'user-1' });
    ...
    it('should return list of workspaces with correct badges', async () => {
        (global as unknown as { mockDbResult: unknown[] }).mockDbResult = [];
        const response = await resolveWorkspaceList({ organizationId: 'org-1', userId: 'user-1' });
