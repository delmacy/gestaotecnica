...
test("evaluateApprovalPolicy - Applicability", async (t) => {
  ...
  await t.test("should not be applicable if workspaceId mismatch", () => {
    const result = evaluateApprovalPolicy({
      policy,
      decisions: [],
      subject: SUBJECT,
  ...
  await t.test("should not be applicable if subject type mismatch", () => {
    const result = evaluateApprovalPolicy({
      policy,
      decisions: [],
      subject: { ...SUBJECT, type: "form_definition" },
      operation: OPERATION,
      workspaceId: WORKSPACE_ID,
    });
    assert.strictEqual(result.applicable, false);
  ...
  await t.test("should not be applicable if policy is inactive", () => {
    const inactivePolicy = createPolicy({ status: "draft" });
    const result = evaluateApprovalPolicy({
...
test("evaluateApprovalPolicy - Mode: none", async (t) => {
